...
class SchedulesController(BaseController):
    ...
    def create(self, **kw):
        try:
            ...
            if not time_slot or not package_type_id:
                self._self._flash('请填写时段和套餐', 'danger')
            ...
            if existing:
                self._self._flash('该时段该套餐的排期已存在', 'warning')
            ...
            self._self._flash('备餐排期创建成功', 'success')
        except Exception as e:
            self._self._flash(f'创建失败: {str(e)}', 'danger')
        redirect(url('/schedules'))
    ...
    def update(self, id, **kw):
        try:
            ...
            if schedule.status == 'completed':
                self._self._flash('已完成的排期不能修改', 'danger')
                redirect(url('/schedules'))
            ...
            self._flash('排期更新成功', 'success')
        except Exception as e:
            self._self._flash(f'更新失败: {str(e)}', 'danger')
        redirect(url('/schedules'))
    ...
    def complete(self, id, **kw):
        try:
            schedule = PreparationSchedule.get(int(id))
            if schedule.status == 'completed':
                self._flash('排期已完成', 'warning')
            else:
                user = self._get_current_user()
                schedule.status = 'completed'
                schedule.prepared_byID = user.id if user else None
                schedule.completed_at = datetime.now()
                self._self._flash('备餐已完成', 'success')
        except Exception as e:
            self._self._flash(f'操作失败: {str(e)}', 'danger')
        redirect(url('/schedules'))
    ...
    def delete(self, id, **kw):
        try:
            schedule = PreparationSchedule.get(int(id))
            if schedule.status == 'completed':
                self._self._flash('已完成的排期不能删除', 'danger')
            else:
                schedule.destroySelf()
                self._flash('排期已删除', 'success')
        except Exception as e:
            self._self._flash(f'删除失败: {str(e)}', 'danger')
        redirect(url('/schedules'))
